...
- The sync currently only reads `fulfillment.inStore` (boolean) and `price.regular` from Kroger's product response. The same response also includes `inventory.stockLevel` (observed value: `"HIGH"` — likely a coarse LOW/MEDIUM/HIGH-style enum) which isn't read or stored anywhere yet — a possible future upgrade from a flat in/out badge to a stock-level indicator, not yet decided on.
